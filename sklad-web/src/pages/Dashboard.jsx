  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="animate-fade-in-up">
        ...
        <p className="text-sm text-slate-400 dark:text-slate-500 mt-1">{new Date().toLocaleDateString("uz-UZ", { weekday: "long", year: "numeric", month: "long", day: "numeric" })}</p>
      </div>

      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <StatCard icon={ShoppingCart} label="Bugungi sotuv" value={formatNum(data.today.amount)} bg="bg-indigo-50 dark:bg-indigo-500/10" color="text-indigo-600" delay="delay-100" onClick={isAdmin ? () => openModal("today") : undefined} />
        <StatCard icon={TrendingUp} label="Oylik sotuv" value={formatNum(data.month.amount)} bg="bg-emerald-50 dark:bg-emerald-500/10" color="text-emerald-600" delay="delay-200" onClick={isAdmin ? () => openModal("month") : undefined} />
        <StatCard icon={CreditCard} label="Umumiy qarz" value={formatNum(data.totalDebt)} bg="bg-red-50 dark:bg-red-500/10" color="text-red-500" delay="delay-300" onClick={isAdmin ? () => openModal("debts") : undefined} show={isAdmin} />
        <StatCard icon={Users} label="Mijozlar" value={data.totalClients} bg="bg-violet-50 dark:bg-violet-500/10" color="text-violet-600" delay="delay-400" onClick={isAdmin ? () => openModal("clients") : undefined} />
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 animate-fade-in-up delay-300">
        {hasChartData && (
          <div className="md:col-span-2 bg-white dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700 p-5">
            <h2 className="text-base font-semibold text-slate-700 dark:text-slate-200 mb-4">Haftalik sotuvlar</h2>
        ...
        <div className={hasChartData ? "space-y-4" : "md:col-span-3 grid grid-cols-1 md:grid-cols-2 gap-4"}>
          {isAdmin && (
            <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700 p-5">
              <div className="flex items-center gap-2 mb-3">
                ...
                <h2 className="text-base font-semibold text-slate-700 dark:text-slate-200">Kam qoldiq</h2>
                {data.lowStockCount > 0 && <span className="ml-auto bg-red-50 dark:bg-red-500/10 text-red-500 text-xs px-2 py-0.5 rounded-full font-medium">{data.lowStockCount} ta</span>}
              </div>
              {lowStock.length === 0 ? (
          ...
          <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700 p-5">
            <div className="flex items-center gap-2 mb-3 cursor-pointer" onClick={() => openModal("recent")}>
              ...
              <h2 className="text-base font-semibold text-slate-700 dark:text-slate-200">Oxirgi sotuvlar</h2>
              <ExternalLink size={14} className="text-slate-400 ml-auto" />
            </div>
            {recentSales.length === 0 ? (
              <p className="text-sm text-slate-400 text-center py-4">Sotuvlar yoq</p>
            ) : (
              <div className="space-y-1">
                {recentSales.map(s => (
                  <div key={s.id} onClick={() => navigate("/sales/" + s.id)} className="flex items-center justify-between text-sm hover:bg-slate-50 dark:hover:bg-slate-700/50 rounded-lg px-2 py-1.5 cursor-pointer">
                    <div className="flex items-center gap-2 min-w-0">
                      ...
                      <span className="text-slate-700 dark:text-slate-300 truncate">{s.client?.name || "Noaniq"}</span>
                    </div>
                    <span className="font-medium text-slate-800 dark:text-white ml-2 flex-shrink-0">{Number(s.totalAmount).toLocaleString()} som</span>
                  </div>
                ))}
              </div>
            )}
      ...
      <QuickModal open={modal === "today" || modal === "month" || modal === "recent"} onClose={() => setModal(null)} title={modal === "today" ? "Bugungi sotuvlar" : modal === "month" ? "Oylik sotuvlar" : "Oxirgi sotuvlar"}>
        {modalLoading ? <div className="flex justify-center py-8"><div className="animate-spin h-8 w-8 border-4 border-indigo-500 border-t-transparent rounded-full" /></div> : modalData.length === 0 ? <p className="text-center text-slate-400 py-8">Sotuvlar yoq</p> : (
            ...
            {modalData.map(s => (
              <div key={s.id} onClick={() => { setModal(null); navigate("/sales/" + s.id); }} className="flex items-center justify-between p-3 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-700/50 cursor-pointer border border-slate-100 dark:border-slate-700">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-medium text-slate-800 dark:text-white text-sm">{s.client?.name || "Noaniq"}</p>
                    <p className="text-xs text-slate-400">{new Date(s.createdAt).toLocaleString()} · {s.user?.name}</p>
                  </div>
                </div>
                <div className="text-right">
                  <p className="font-bold text-slate-800 dark:text-white text-sm">{Number(s.totalAmount).toLocaleString()} som</p>
      ...
      <QuickModal open={modal === "debts"} onClose={() => setModal(null)} title="Nasiyalar">
        {modalLoading ? <div className="flex justify-center py-8"><div className="animate-spin h-8 w-8 border-4 border-indigo-500 border-t-transparent rounded-full" /></div> : modalData.length === 0 ? <p className="text-center text-slate-400 py-8">Nasiyalar yoq</p> : (
          <div className="space-y-2">
            {modalData.map(d => (
              <div key={d.id} onClick={() => { setModal(null); navigate("/debts"); }} className="flex items-center justify-between p-3 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-700/50 cursor-pointer border border-slate-100 dark:border-slate-700">
                <div>
                  ...
                  <p className="text-xs text-slate-400">{d.client?.phone} · {new Date(d.dueDate).toLocaleDateString()}</p>
                </div>
                <div className="text-right">
                  <p className="font-bold text-red-500 text-sm">{(Number(d.amount) - Number(d.paid)).toLocaleString()} som</p>
      ...
      <QuickModal open={modal === "clients"} onClose={() => setModal(null)} title="Mijozlar">
        {modalLoading ? <div className="flex justify-center py-8"><div className="animate-spin h-8 w-8 border-4 border-indigo-500 border-t-transparent rounded-full" /></div> : modalData.length === 0 ? <p className="text-center text-slate-400 py-8">Mijozlar yoq</p> : (
          <div className="space-y-2">
            {modalData.map(c => (
              <div key={c.id} onClick={() => { setModal(null); navigate("/clients/" + c.id); }} className="flex items-center justify-between p-3 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-700/50 cursor-pointer border border-slate-100 dark:border-slate-700">
                ...
                <Badge variant={c.isBlocked ? "red" : "green"}>{c.isBlocked ? "Bloklangan" : "Faol"}</Badge>
              </div>
            ))}
          </div>
        )}
