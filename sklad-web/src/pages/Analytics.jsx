    ...
    <div className="fixed inset-0 z-[70] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white dark:bg-slate-800 rounded-xl shadow-xl w-full max-w-sm animate-fade-in">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 dark:border-slate-700">
          <h2 className="text-lg font-semibold text-slate-800 dark:text-white">Товар</h2>
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-slate-50 dark:bg-slate-700 rounded-lg p-3">
              ...
              <p className="font-bold text-slate-800 dark:text-white text-sm">{Number(product.sellPrice).toLocaleString("ru-RU")} сом</p>
            </div>
            <div className="bg-slate-50 dark:bg-slate-700 rounded-lg p-3">
              <p className="text-xs text-slate-400 mb-1">Цена прихода</p>
              <p className="font-bold text-slate-800 dark:text-white text-sm">{Number(product.buyPrice).toLocaleString("ru-RU")} сом</p>
            </div>
            <div className="bg-slate-50 dark:bg-slate-700 rounded-lg p-3">
            ...
            <div className="bg-indigo-50 dark:bg-indigo-500/10 rounded-lg p-3">
              <p className="text-xs text-slate-400 mb-1">Прибыль (1 шт)</p>
              <p className="font-bold text-emerald-600 text-sm">{(Number(product.sellPrice)-Number(product.buyPrice)).toLocaleString("ru-RU")} сом</p>
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white dark:bg-slate-800 rounded-xl shadow-xl w-full max-w-lg max-h-[85vh] flex flex-col animate-fade-in">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 dark:border-slate-700">
          <h2 className="text-lg font-semibold text-slate-800 dark:text-white">Детали продажи</h2>
          <button onClick={onClose} className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-200"><X size={20} /></button>
        </div>
        <div className="overflow-y-auto flex-1 p-5 space-y-4">
          {loading ? <div className="flex justify-center py-8"><div className="animate-spin h-8 w-8 border-4 border-indigo-500 border-t-transparent rounded-full" /></div> : (
            <>
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-slate-50 dark:bg-slate-700 rounded-lg p-3"><p className="text-xs text-slate-400 mb-1">Клиент</p><p className="font-medium text-slate-800 dark:text-white text-sm">{s.client?.name || "Неизвестно"}</p></div>
                <div className="bg-slate-50 dark:bg-slate-700 rounded-lg p-3"><p className="text-xs text-slate-400 mb-1">Продавец</p><p className="font-medium text-slate-800 dark:text-white text-sm">{s.user?.name}</p></div>
                <div className="bg-slate-50 dark:bg-slate-700 rounded-lg p-3"><p className="text-xs text-slate-400 mb-1">Оплата</p><Badge variant="blue">{PAYMENT_LABELS[s.paymentType]}</Badge></div>
                <div className="bg-slate-50 dark:bg-slate-700 rounded-lg p-3"><p className="text-xs text-slate-400 mb-1">Статус</p><Badge variant={STATUS[s.status]?.variant}>{STATUS[s.status]?.label}</Badge></div>
                <div className="bg-slate-50 dark:bg-slate-700 rounded-lg p-3"><p className="text-xs text-slate-400 mb-1">Дата</p><p className="font-medium text-slate-800 dark:text-white text-sm">{new Date(s.createdAt).toLocaleString("ru-RU")}</p></div>
                <div className="bg-indigo-50 dark:bg-indigo-500/10 rounded-lg p-3"><p className="text-xs text-slate-400 mb-1">Итого</p><p className="font-bold text-indigo-600 dark:text-indigo-400 text-sm">{Number(s.totalAmount).toLocaleString("ru-RU")} сом</p></div>
              </div>
                  ...
                  <div className="space-y-2">
                    {s.items.map((item, i) => (
                      <div key={i} className="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-700 rounded-lg text-sm">
                        <div><p className="font-medium text-slate-800 dark:text-white">{item.product?.name}</p><p className="text-xs text-slate-400">{item.quantity} x {Number(item.price).toLocaleString("ru-RU")} сом</p></div>
        ...
        <div className="relative bg-white dark:bg-slate-800 rounded-xl shadow-xl w-full max-w-lg max-h-[85vh] flex flex-col animate-fade-in">
          <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 dark:border-slate-700">
            <div>
              ...
              <p className="text-xs text-slate-400">{seller.count} продаж · {Number(seller.totalAmount).toLocaleString("ru-RU")} сом</p>
            </div>
            <button onClick={onClose} className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-200"><X size={20} /></button>
          </div>
          <div className="overflow-y-auto flex-1 p-4">
            {loading ? <div className="flex justify-center py-8"><div className="animate-spin h-8 w-8 border-4 border-indigo-500 border-t-transparent rounded-full" /></div>
            : sales.length === 0 ? <p className="text-center text-slate-400 py-8">Продаж нет</p>
            : <div className="space-y-2">
                {sales.map(s => (
                  <div key={s.id} onClick={() => setSelectedSale(s)} className="flex items-center justify-between p-3 rounded-lg bg-slate-50 dark:bg-slate-700 border border-slate-100 dark:border-slate-600 cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-600">
                    <div><p className="font-medium text-slate-800 dark:text-white text-sm">{s.client?.name || "Неизвестно"}</p><p className="text-xs text-slate-400">{new Date(s.createdAt).toLocaleString("ru-RU")} · {PAYMENT_LABELS[s.paymentType]}</p></div>
                    <div className="text-right"><p className="font-bold text-slate-800 dark:text-white text-sm">{Number(s.totalAmount).toLocaleString("ru-RU")} сом</p><Badge variant={STATUS[s.status]?.variant}>{STATUS[s.status]?.label}</Badge></div>
                  </div>
                ))}
              </div>}
          </div>
...
export default function Analytics() {
  ...
  const openProduct = async (name) => {
    ...
    } catch { /* товар не найден — игнорируем */ }
  };
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="animate-fade-in-up">
        <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Аналитика</h1>
        <p className="text-sm text-slate-400 dark:text-slate-500 mt-1">Статистика продаж и товаров</p>
      </div>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700 p-5 animate-fade-in-up delay-100">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-base font-semibold text-slate-700 dark:text-slate-200">График продаж</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700 p-5 animate-fade-in-up delay-200">
          <h2 className="text-base font-semibold text-slate-700 dark:text-slate-200 mb-4">Топ товаров</h2>
          {topProducts.length === 0 ? <p className="text-slate-400 text-center py-8 text-sm">Нет данных</p> : (
              ...
              {(() => { const max = Math.max(...topProducts.map(p => p.totalAmount)); return topProducts.map((p, i) => (
                <div key={i} className="cursor-pointer" onClick={() => openProduct(p.name)}>
                  <div className="flex justify-between text-sm mb-1">
                    ...
                    <span className="text-slate-500 dark:text-slate-400">{Number(p.totalAmount).toLocaleString("ru-RU")} сом</span>
                  </div>
        ...
        <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700 p-5 animate-fade-in-up delay-300">
          <h2 className="text-base font-semibold text-slate-700 dark:text-slate-200 mb-4">Рейтинг продавцов</h2>
          {sellers.length === 0 ? <p className="text-slate-400 text-center py-8 text-sm">Нет данных</p> : (
            <div className="space-y-2">
              {sellers.map((s, i) => (
                <div key={i} onClick={() => setSelectedSeller(s)} className="flex items-center gap-3 p-3 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-700/50 cursor-pointer">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-slate-400">{s.count} продаж</p>
                  </div>
                  <span className="text-sm font-semibold text-emerald-600 dark:text-emerald-400">{Number(s.totalAmount).toLocaleString("ru-RU")} сом</span>
                </div>
              ))}
            </div>
          )}
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700 p-5 animate-fade-in-up delay-300">
        <h2 className="text-base font-semibold text-slate-700 dark:text-slate-200 mb-4">Топ товаров по прибыли</h2>
        {topProfit.length === 0 ? <p className="text-slate-400 text-center py-8 text-sm">Нет данных</p> : (
          <div className="space-y-3">
            {(() => { const max = Math.max(...topProfit.filter(p => p.profit > 0).map(p => p.profit), 1); return topProfit.filter(p => p.profit > 0).map((p, i) => (
              <div key={i} className="cursor-pointer" onClick={() => openProduct(p.name)}>
                <div className="flex justify-between text-sm mb-1">
                  ...
                  <span className="text-emerald-600 dark:text-emerald-400 font-semibold">{Number(p.profit).toLocaleString("ru-RU")} сом</span>
                </div>
                <div className="h-2 bg-slate-100 dark:bg-slate-700 rounded-full overflow-hidden">
