...
export default function AuditLog() {
  ...
  const columns = [
    { title: 'Sana', key: 'createdAt', render: (v) => <span className="text-xs text-gray-500">{new Date(v).toLocaleString()}</span> },
    ...
    { title: 'Ob\'ekt', key: 'entity', render: (v) => <span className="text-gray-500">{v}</span> },
    ...
    <div className="space-y-4">
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between bg-white rounded-xl shadow-sm px-4 py-3">
          <p className="text-sm text-gray-500">Jami: {total} ta yozuv</p>
          <div className="flex gap-2">
            <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1} className="px-3 py-1.5 text-sm border rounded-lg hover:bg-gray-50 disabled:opacity-50">Oldingi</button>
            <span className="px-3 py-1.5 text-sm bg-blue-600 text-white rounded-lg">{page} / {totalPages}</span>
            <button onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={page === totalPages} className="px-3 py-1.5 text-sm border rounded-lg hover:bg-gray-50 disabled:opacity-50">Keyingi</button>
          </div>
        </div>
      )}
    </div>
