...
export default function SaleDetail() {
  ...
  if (!sale) return <div className="text-center py-20 text-gray-400">Topilmadi</div>;
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-4">
      <div className="flex items-center gap-3">
        <button onClick={() => navigate('/sales')} className="p-2 hover:bg-gray-100 rounded-lg transition-colors">
          <ArrowLeft size={20} />
        </button>
        <h1 className="text-2xl font-bold text-gray-800 flex-1">Sotuv #{sale.id.slice(-6).toUpperCase()}</h1>
      ...
      <div className="bg-white rounded-xl shadow-sm overflow-hidden">
        <div className="px-5 py-3 border-b bg-gray-50">
          <h2 className="font-semibold text-gray-700">Mahsulotlar</h2>
        </div>
        <table className="w-full text-sm">
          <thead className="bg-gray-50">
            <tr>
              <th className="px-5 py-2 text-left text-xs text-gray-500">Nomi</th>
              <th className="px-5 py-2 text-right text-xs text-gray-500">Miqdor</th>
              <th className="px-5 py-2 text-right text-xs text-gray-500">Narx</th>
              <th className="px-5 py-2 text-right text-xs text-gray-500">Jami</th>
            </tr>
          </thead>
        ...
        <div className="px-5 py-4 border-t space-y-2 bg-gray-50">
          ...
          {Number(sale.discount) > 0 && (
            <div className="flex justify-between text-sm">
              <span className="text-gray-500">Skidka</span>
              <span className="text-red-500">-{Number(sale.discount).toLocaleString()} so'm</span>
            </div>
          )}
          <div className="flex justify-between font-bold text-base border-t pt-2">
            <span>Tolov summasi</span>
            <span className="text-blue-600">{Number(sale.totalAmount).toLocaleString()} so'm</span>
          </div>
        </div>
      </div>
