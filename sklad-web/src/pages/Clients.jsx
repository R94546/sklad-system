...
import { Plus, Edit, Ban, X, Phone, MapPin } from "lucide-react";
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white dark:bg-slate-800 rounded-xl shadow-xl w-full max-w-lg max-h-[85vh] flex flex-col animate-fade-in">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 dark:border-slate-700">
          <h2 className="text-lg font-semibold text-slate-800 dark:text-white">О клиенте</h2>
        ...
        <div className="overflow-y-auto flex-1 p-5 space-y-4">
          <div className="flex items-center gap-4">
            ...
            <Badge className="ml-auto" variant={client.isBlocked ? "red" : "green"}>{client.isBlocked ? "Заблокирован" : "Активен"}</Badge>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="bg-red-50 dark:bg-red-500/10 rounded-lg p-3 text-center">
              <p className="text-xs text-slate-400 mb-1">Общий долг</p>
              <p className="text-xl font-bold text-red-500">{totalDebt.toLocaleString("ru-RU")} сом</p>
            </div>
          </div>
          ...
          {loading ? <div className="flex justify-center py-4"><div className="animate-spin h-6 w-6 border-4 border-indigo-500 border-t-transparent rounded-full" /></div> : (
            <>
              {sales.length > 0 && (
                <div>
                  <p className="text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">Последние продажи</p>
                  <div className="space-y-2">
                    {sales.slice(0, 5).map(s => (
                      <div key={s.id} className="flex justify-between p-2 bg-slate-50 dark:bg-slate-700 rounded-lg text-sm">
                        <span className="text-slate-500 dark:text-slate-400">{new Date(s.createdAt).toLocaleDateString("ru-RU")}</span>
                        <span className="font-medium text-slate-800 dark:text-white">{Number(s.totalAmount).toLocaleString("ru-RU")} сом</span>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {debts.filter(d => d.status !== "PAID").length > 0 && (
                <div>
                  <p className="text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">Долги</p>
                  <div className="space-y-2">
                    {debts.filter(d => d.status !== "PAID").map(d => (
                      <div key={d.id} className="flex justify-between p-2 bg-red-50 dark:bg-red-500/10 rounded-lg text-sm">
                        <span className="text-slate-500 dark:text-slate-400">{new Date(d.dueDate).toLocaleDateString("ru-RU")}</span>
                        <span className="font-medium text-red-500">{(Number(d.amount)-Number(d.paid)).toLocaleString("ru-RU")} сом</span>
                      </div>
                    ))}
                  </div>
                </div>
...
export default function Clients() {
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true);
    try {
      if (editing) { await api.put("/clients/" + editing.id, form); toast.success("Обновлено"); }
      else { await api.post("/clients", form); toast.success("Создано"); }
      setModal(false); load();
  ...
  const handleBlock = async (e, c) => {
    e.stopPropagation();
    try { await api.patch("/clients/" + c.id + "/block", { isBlocked: !c.isBlocked }); toast.success(c.isBlocked ? "Разблокирован" : "Заблокирован"); load(); }
    catch { toast.error("Ошибка"); }
  };
  ...
  return (
    <div className="space-y-6 animate-fade-in">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-slate-50 dark:bg-slate-700/50 border-b border-slate-100 dark:border-slate-700">
              <tr>
                {["Имя","Телефон","Адрес","Статус","Действия"].map((h,i) => (
            ...
            <tbody className="divide-y divide-slate-50 dark:divide-slate-700">
              {loading ? (
                ...
                <tr><td colSpan={99} className="py-2"><EmptyState type="clients" title="Клиенты не найдены" desc="Нажмите + чтобы добавить клиента" /></td></tr>
              ) : clients.map(c => (
                <tr key={c.id} onClick={() => setSelected(c)} className="hover:bg-slate-50 dark:hover:bg-slate-700/30 cursor-pointer">
                  ...
                  <td className="px-4 py-3 text-center"><Badge variant={c.isBlocked ? "red" : "green"}>{c.isBlocked ? "Заблокирован" : "Активен"}</Badge></td>
      ...
      <Modal open={modal} onClose={() => setModal(false)} title={editing ? "Редактировать клиента" : "Новый клиент"}>
        <form onSubmit={handleSubmit} className="space-y-4">
          <Input label="Имя и фамилия" value={form.name} onChange={e => setForm({...form, name: e.target.value})} required />
          ...
          <div className="flex flex-col gap-1">
            <label className="text-sm font-medium text-slate-700 dark:text-slate-300">Заметка</label>
