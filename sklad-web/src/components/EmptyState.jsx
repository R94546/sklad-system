...
export default function EmptyState({ type = "default", title, description, action }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center py-16 px-4 text-center">
      ...
      <h3 className="text-base font-semibold text-slate-700 dark:text-slate-300 mb-1">{title || "Malumot topilmadi"}</h3>
      <p className="text-sm text-slate-400 dark:text-slate-500 max-w-xs mb-4">{description || "Hozircha bu yerda hech narsa yoq"}</p>
      {action && (
